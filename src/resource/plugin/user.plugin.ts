...
export default {
    ...
    oAuth: {
        getUser: async (_accessToken: { tokenType: 'Bearer' | 'Basic', accessToken: string & { __brand: 'TOKEN' } }): Promise<
            { success: true, id: string, username?: string, name?: string, birthdate?: Date, gender?: 'male' | 'female', nickname?: string, emailAddress?: string, phoneNumber?: string, image?: string }
            | { success: false, error?: Error }
        > => {
            try {
                ...
                return {
                    ...
                    image: _result.data.data.image ? _result.data.data.image : undefined
                }
